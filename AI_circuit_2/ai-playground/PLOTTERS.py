...
def plotPerformance(X_test, Y_test, model):
    ...
    # voy a hacer una chanchadita y cambiar ROC por precission recall...
    #tprr, fprr, tresholdsr = metrics.precision_recall_curve(y_true, y_scoresrandom) # XANXO 
    plt.plot(fprr,tprr, label = 'random')
    y_scores = model.predict_proba(X_test)
    fpr, tpr, thresholds = metrics.roc_curve(y_true, y_scores)
    # voy a hacer una chanchadita y cambiar ROC por precission recall...
    #tpr, fpr, tresholds = metrics.precision_recall_curve(y_true, y_scores) # XANXO 
