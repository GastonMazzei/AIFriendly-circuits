...
def main():    
    ...
    df = pd.read_excel(directorio+'/TRAIN.xlsx')
    dataset = df.to_numpy()
    #                     custom_model=False
    nNetwork = AI_model(dataset,False)
    nNetwork.trainModel()
    #nNetwork.model.save(directorio + '/model.h5')
    #joblib.dump(nNetwork.scaler, directorio + '/scaler.dat') 
    result = {}
    result['confidence'] = nNetwork.confidence
    # PLOTTING:
    plotPerformance(nNetwork.X_test, nNetwork.Y_test, nNetwork.model)
    plt.savefig(directorio + '/figPerf.png', bbox_inches='tight')
    plt.close()
    plotLoss(nNetwork.history)
    plt.savefig(directorio + '/figLoss.png', bbox_inches='tight')
    plt.close()
    plotAcc(nNetwork.history)
    plt.savefig(directorio + '/figAcc.png', bbox_inches='tight')
    plt.close()
